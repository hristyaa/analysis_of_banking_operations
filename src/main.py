...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            user_input = input(f'''{greeting()} Добро пожаловать в приложение для анализа транзакций из Excel-файла!
...
''')
            if user_input in ['1', '2', '3']:
                if user_input == '1':
                    ...
                    while True:
                        user_date = input('''Введите дату для анализа и вывода на веб-страницах в формате YYYY-MM-DD HH:MM:SS
''')
                        try:
...
Введите данные для анализа:''')
                            while True:
                                ...
                                if len(user_year) == 4 and len(user_month) == 2 and 0 < int(user_month) <= 12 and user_year.isdigit():
                                    break
                                else:
                                    print('Данные введены некорректно')
                            result = get_analysis_categories_of_increased_cashback(operations, user_year, user_month)
                            print(f'''По введенным параметрам ({user_year} год, {user_month} месяц) наиболее выгодные категории кэшбэка:'
{result}''')
...
Сервис показывает сумму, которую удалось бы отложить в «Инвесткопилку»''')
                            while True:
                                limit = int(input('''Выберите шаг округления в рублях 10, 50 или 100
'''))
                            ...
                            while True:
                                month = input('''Введите год и месяц для которого рассчитывается отложенная сумма (например, 2018-03)
''')
                                date = month.split("-")
                            ...
                            print(f'За {month} при округлении ваших трат до {limit} руб. '
...
за последние три месяца (от переданной или текущей даты)''')
                    while True:
                        date_for_report = input('Отчет "Траты по категории" необходим для текущей даты? да/нет\n').lower()
                        if date_for_report == 'да':
                            date_report = None
                            break
                        elif date_for_report == 'нет':
                            while True:
                                date_report = input('''Введите дату для формирования отчета (например, 10.10.2020)
''')
                                try:
                            ...
                            if enabled_input == 'да':
                                ...
                                while True:
                                    ...
                                    if file_input == 'да':
                                        ...
                                        while True:
                                            file_name = input('Введите название файла (с расширением, например report.json): ')
                                            file_name_ext = file_name.split('.')
                                            if ('.' + file_name_ext[1]) in valid_extensions:
                                                filename = file_name
                                                break
                                            else:
                                                print(f"Неподдерживаемый формат! Допустимы: {', '.join(valid_extensions)}")
                                        filename = file_name
                                        break
                                ...
                                decorated_func_save = save_to_file(filename=filename, enabled=True)(spending_by_category)
                                decorated_func_save(transactions, category, date_report)
                                print(f'Отчет сохранен в файл: {os.path.join(os.path.dirname(os.getcwd()), "reports", filename)}')
                                break
